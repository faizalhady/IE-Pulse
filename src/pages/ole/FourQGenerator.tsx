    ...
    <div className="flex-1 flex items-center justify-center p-8">
      <div className="w-full max-w-lg space-y-6">
        ...
        <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 text-xs text-muted-foreground flex items-start gap-2">
          ...
          <span>Trend uses latest 13 weeks. Q2 Paretos use last 4 actual weeks averaged.</span>
        </div>
...
function SmallPareto({ title, data, loading, height=180 }: { title: string; data: ParetoBar[]; loading: boolean; height?: number }) {
  if (loading) return <div className="rounded-lg bg-muted/40 animate-pulse" style={{ height }} />;
  if (!data.length) return (
    <div className="flex items-center justify-center border border-border rounded-lg text-xs text-muted-foreground" style={{ height }}>No data</div>
  );
  return (
    <div className="border border-border rounded-lg p-3 flex flex-col gap-1">
      ...
      <div style={{ height }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top:4, right:28, left:-20, bottom:32 }}>
            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border))" />
            <XAxis dataKey="name" tick={{ fontSize:9 }} tickLine={false} axisLine={false}
              interval={0} angle={-25} textAnchor="end" height={38} />
            <YAxis yAxisId="left" tick={{ fontSize:9 }} tickLine={false} axisLine={false} tickFormatter={v=>`${v.toFixed(0)}%`} />
...
function Q2Section({ aggregateRows, scopeWorkcells, scopePlant, compact=false }: {
  ...
  if (compact) {
    // In compact mode, fill the parent container height
    return (
      <div className="flex gap-2 h-full">
        <div className="flex-1 min-w-0 flex flex-col">
          <SmallPareto title="Man-hrs Loss Distribution" data={pareto1} loading={loading} height={272} />
        </div>
        <div className="flex-1 min-w-0 flex flex-col gap-2">
          <SmallPareto title={`Top 3 WC — ${top1Cat||'#1 Loss'}`} data={pareto2} loading={loading} height={130} />
          <SmallPareto title={`Top 3 WC — ${top2Cat||'#2 Loss'}`} data={pareto3} loading={loading} height={130} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      <p className="text-[11px] text-muted-foreground">Avg of last 4 actual weeks · loss % of total input hours</p>
      <SmallPareto title="Man-hrs Loss Distribution" data={pareto1} loading={loading} height={200} />
      <div className="grid grid-cols-2 gap-3">
        <SmallPareto title={`Top 3 WC — ${top1Cat||'#1 Loss'}`} data={pareto2} loading={loading} height={180} />
        <SmallPareto title={`Top 3 WC — ${top2Cat||'#2 Loss'}`} data={pareto3} loading={loading} height={180} />
      </div>
    </div>
  );
}
...
function Q3Table({ aggregateRows, scopeWorkcells, scopePlant, isPrint=false }: {
      ...
      try {
        ...
        const results = await Promise.all(actWeeks.map(async w => {
          try {
            ...
            return { week_label: w.week_label, nva, lunch, mfg_dt: mfg, unexplained: unexp,
              total: parseFloat((nva + lunch + mfg + unexp).toFixed(2)) } as MhWeekData;
          } catch { return null; }
        }));
  ...
  return (
    <div className="overflow-x-auto w-full">
      <table className={cn('w-full text-left border-collapse', fs)}>
        <thead>
          <tr className="bg-blue-600 text-white uppercase tracking-wider">
            <th className={cn(ph, 'border border-blue-700 font-semibold sticky left-0 bg-blue-600 z-10 whitespace-nowrap min-w-[160px]')}>
              Man Hrs Distribution
            </th>
            {weekData.map(w=>(
              <th key={w.week_label} className={cn(ph, 'border border-blue-700 text-right font-semibold whitespace-nowrap')}>
                {fmtWeekLabel(w.week_label)}
              </th>
            ))}
            <th className={cn(ph, 'border border-blue-700 text-right font-bold whitespace-nowrap bg-blue-800')}>
              Avg 4 Wks
            </th>
          </tr>
        </thead>
        <tbody>
          {MH_ROWS.map(row=>{
            ...
            return (
              <tr key={row.key} className="border-b border-border hover:bg-muted/20 transition-colors">
                <td className={cn(px, 'border border-border font-semibold sticky left-0 bg-card z-10 whitespace-nowrap')}>
                ...
                <td className={cn(px, 'border border-blue-200 text-right font-mono font-bold tabular-nums bg-blue-50 text-blue-700')}>
          ...
          <tr className="bg-blue-50 font-bold text-blue-900">
            <td className={cn(px, 'border border-blue-200 sticky left-0 bg-blue-50 z-10 uppercase tracking-wider text-blue-900')}>Total</td>
            {weekData.map(w=>(
              <td key={w.week_label} className={cn(px, 'border border-blue-200 text-right font-mono tabular-nums')}>
                {w.total.toFixed(2)}%
              </td>
            ))}
            <td className={cn(px, 'border border-blue-200 text-right font-mono tabular-nums bg-blue-100 text-blue-800')}>
...
function Q4Table({ actions, isPrint=false }: { actions: ActionItem[]; isPrint?: boolean }) {
  return (
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-[98vw] w-[98vw] h-[98vh] p-0 flex flex-col gap-0 border border-border bg-card shadow-2xl rounded-xl overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden bg-muted/40 p-4 min-h-0">
          <div className="bg-white text-black h-full w-full rounded-lg shadow-sm p-5 flex flex-col gap-3 overflow-hidden" style={{ minWidth:900 }}>

            {/* Report title bar */}
            <div className="border-b-2 border-blue-700 pb-2 flex items-center justify-between flex-shrink-0">
              <h1 className="text-lg font-bold uppercase tracking-wide">{title}</h1>
              <span className="text-xs font-semibold uppercase tracking-widest text-gray-500">JABIL 4Q REPORT</span>
            </div>
            ...
            <div className="flex-1 grid grid-cols-2 grid-rows-2 gap-3 min-h-0 overflow-hidden">
              ...
              {/* Q1 */}
              <div className="border border-gray-200 rounded-lg p-3 flex flex-col min-h-0 overflow-hidden">
                <div className="flex items-center gap-2 mb-1 flex-shrink-0">
                  <span className="text-white w-5 h-5 rounded flex items-center justify-center text-xs font-bold bg-blue-600">1</span>
                  <span className="text-xs font-bold uppercase text-blue-600">OLE Trend + Projection · {trendScope}</span>
                </div>
              ...
              <div className="border border-gray-200 rounded-lg p-3 flex flex-col min-h-0 overflow-hidden">
                <div className="flex items-center gap-2 mb-1 flex-shrink-0">
                  <span className="text-white w-5 h-5 rounded flex items-center justify-center text-xs font-bold bg-orange-500">2</span>
                  <span className="text-xs font-bold uppercase text-orange-500">Man-Hours Loss Pareto</span>
                </div>
              ...
              <div className="border border-gray-200 rounded-lg p-3 flex flex-col min-h-0 overflow-hidden">
                <div className="flex items-center gap-2 mb-1 flex-shrink-0">
                  <span className="text-white w-5 h-5 rounded flex items-center justify-center text-xs font-bold bg-blue-500">3</span>
                  <span className="text-xs font-bold uppercase text-blue-500">Paynter Chart</span>
                </div>
              ...
              <div className="border border-gray-200 rounded-lg p-3 flex flex-col min-h-0 overflow-hidden">
                <div className="flex items-center gap-2 mb-1 flex-shrink-0">
                  <span className="text-white w-5 h-5 rounded flex items-center justify-center text-xs font-bold bg-emerald-500">4</span>
                  <span className="text-xs font-bold uppercase text-emerald-500">Action Plan</span>
                </div>
                <div className="flex-1 overflow-auto min-h-0">
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        {tab==='editor'&&(
          <>
              ...
              <div className="max-w-5xl mx-auto space-y-12 pb-16">
                ...
                <div className="space-y-4">
                  <h2 className="text-xl font-bold uppercase flex items-center gap-2 text-primary">
                    <span className="text-primary-foreground w-7 h-7 rounded flex items-center justify-center text-sm bg-primary">1</span>
                    OLE Trend + Projection
                    <span className="text-sm font-normal text-muted-foreground normal-case ml-2">— {trendScope}</span>
                  </h2>
                ...
                <div className="space-y-4">
                  <h2 className="text-xl font-bold uppercase flex items-center gap-2 text-orange-500">
                    <span className="text-white w-7 h-7 rounded flex items-center justify-center text-sm bg-orange-500">2</span>
                    Man-Hours Loss Pareto
                  </h2>
                  <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  <h2 className="text-xl font-bold uppercase flex items-center gap-2 text-blue-500">
                    <span className="text-white w-7 h-7 rounded flex items-center justify-center text-sm bg-blue-500">3</span>
                    Paynter Chart
                  </h2>
                ...
                <div className="space-y-4">
                  <h2 className="text-xl font-bold uppercase flex items-center gap-2 text-emerald-500">
                    <span className="text-white w-7 h-7 rounded flex items-center justify-center text-sm bg-emerald-500">4</span>
                    Action Plan
                  </h2>
                  <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
            ...
            <div className={cn('border-l border-border bg-card/95 backdrop-blur-md transition-all duration-300 ease-in-out shadow-2xl flex flex-col flex-shrink-0 z-10', rightOpen?'w-[460px]':'w-12')}>
              <div
