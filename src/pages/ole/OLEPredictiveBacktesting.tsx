...
function ema(values: number[], period: number): number | null {
  ...
  return e; // Return true EMA of the array
}

function linearReg(values: number[], stepsAhead = 1): number | null {
  ...
  // Last data point is at index (n - 1). Project stepsAhead from there:
...
function buildChartData(rows: OleWeeklyResult[], projectionWeeks: number): ChartPoint[] {
  ...
  // 1. Strict T+1 Historical Backtesting
  // The formulas are ONLY allowed to see data up to (but not including) index i.
  const points: ChartPoint[] = valid.map((r, i) => ({
    week_label: r.week_label,
    actual: r.ole_pct,
    sma3: i >= 3 ? sma(actuals.slice(0, i), 3) : null,
    sma5: i >= 5 ? sma(actuals.slice(0, i), 5) : null,
    wma3: i >= 3 ? wma(actuals.slice(0, i), 3) : null,
    ema_fast: i >= 1 ? ema(actuals.slice(0, i), 3) : null,
    ema_slow: i >= 1 ? ema(actuals.slice(0, i), 9) : null,
    cma: i >= 1 ? sma(actuals.slice(0, i), i) : null,
    linear_reg: i >= 2 ? linearReg(actuals.slice(0, i)) : null,
    projected: false,
  }));

  // 2. Dynamic Future Projection
  const lastRow = valid[valid.length - 1];
  let projYear = lastRow.iso_year;
  let projWeek = lastRow.iso_week;

  // Maintain separate memory arrays to feed predictions back into themselves
  const sma3_data = [...actuals];
  const sma5_data = [...actuals];
  const wma3_data = [...actuals];
  const ema_fast_data = [...actuals];
  const ema_slow_data = [...actuals];
  const cma_data = [...actuals];
  ...
  for (let p = 1; p <= projectionWeeks; p++) {
    ...
    // Predict next step based on historical + previous generated predictions
    const s3 = sma(sma3_data, 3);
    const s5 = sma(sma5_data, 5);
    const w3 = wma(wma3_data, 3);
    const ef = ema(ema_fast_data, 3);
    const es = ema(ema_slow_data, 9);
    const cm = sma(cma_data, cma_data.length);
    const lr = linearReg(actuals, p); // Linear Regression projects outward from original base

    // Append predictions back into memory for the next loop (walking forward)
    if (s3 !== null) sma3_data.push(s3);
    if (s5 !== null) sma5_data.push(s5);
    if (w3 !== null) wma3_data.push(w3);
    if (ef !== null) ema_fast_data.push(ef);
    if (es !== null) ema_slow_data.push(es);
    if (cm !== null) cma_data.push(cm);
...
function PredictiveChart({
  ...
  wc: string;
  rows: OleWeeklyResult[];
  projectionWeeks: number;
  activeFormulas: Set<FormulaKey>;
}) {
  const isAll = wc === 'All Workcells';
  const predictions = useOlePredictions({ workcell: wc, projection_weeks: projectionWeeks }, isAll);

  const chartData = useMemo(() => {
    const base = buildChartData(rows, projectionWeeks);
    if (isAll || !predictions.data || predictions.data.length === 0) return base;
    return base.map(b => {
      const pred = predictions.data.find(p => p.week_label === b.week_label);
      if (pred) {
        return { ...b, arima: pred.arima, hw: pred.hw };
      }
      return b;
    });
  }, [rows, projectionWeeks, predictions.data, isAll]);

  const firstProjected = chartData.find(p => p.projected)?.week_label;
  if (chartData.length === 0) return null;

  // Best formula by lowest MAE
  const maes = ALL_FORMULA_KEYS.map(f => ({ f, m: mae(chartData, f as any) })).filter(x => x.m !== null);
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {ALL_FORMULA_KEYS.filter(f => !isAll || (f !== 'arima' && f !== 'hw')).map(f => {
              const m = mae(chartData, f as any);
          ...
          <ComposedChart data={chartData} margin={{ top: 10, right: 16, bottom: 4, left: 0 }}>
            ...
            {firstProjected && (
              <ReferenceArea
                ...
                strokeOpacity={0}
                label={{ value: 'PREDICTION', position: 'insideTop', fill: 'hsl(var(--primary))', fontSize: 10, fontWeight: 700, offset: 10 }}
              />
            )}
            ...
            {/* Formula lines */}
            {ALL_FORMULA_KEYS.filter(f => activeFormulas.has(f)).filter(f => !isAll || (f !== 'arima' && f !== 'hw')).map(f => (
              <Line
                key={f}
                dataKey={f}
                ...
                strokeDasharray={f === 'linear_reg' ? '5 3' : f === 'arima' || f === 'hw' ? '3 3' : undefined}
                dot={false}
                connectNulls
              />
            ))}
...
export default function OLEPredictiveBacktesting() {
